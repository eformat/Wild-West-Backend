...
public class PlatformObjectHelper {
  ...
  private List<PlatformObject> getPods() {
    ...
    for (Pod currPod : pods) {
      thePods.add(new PlatformObject(currPod.getMetadata().getUid(),
          currPod.getMetadata().getName(), "POD"));
    }
    return thePods;
  }
  ...
  private List<PlatformObject> getBuilds() {
    ...
    for (Build currBuild : builds) {
      theBuilds.add(new PlatformObject(currBuild.getMetadata().getUid(),
          currBuild.getMetadata().getName(), "BUILD"));
    }
  ...
  private List<PlatformObject> getDeploymentConfigs() {
    ...
    for (DeploymentConfig currConfig : deploymentConfigs) {
      theDeployments.add(new PlatformObject(currConfig.getMetadata().getUid(),
          currConfig.getMetadata().getName(), "DEPLOYMENT_CONFIG"));
    }
  ...
  private List<PlatformObject> getBuildConfigs() {
    ...
    for (BuildConfig currConfig : theItems) {
      theList.add(new PlatformObject(currConfig.getMetadata().getUid(),
          currConfig.getMetadata().getName(), "BUILD_CONFIG"));
    }
    return theList;
  }
  ...
  private List<PlatformObject> getPVs() {
    ...
    for (PersistentVolumeClaim currConfig : theItems) {
      theList.add(new PlatformObject(currConfig.getMetadata().getUid(),
          currConfig.getMetadata().getName(), "PVC"));
    }
    return theList;
  }
  ...
  private List<PlatformObject> getServices() {
    ...
    for (Service currConfig : theItems) {
      theList.add(new PlatformObject(currConfig.getMetadata().getUid(),
          currConfig.getMetadata().getName(), "SERVICE"));
    }
    return theList;
  }
  ...
  private List<PlatformObject> getRoutes() {
    ...
    for (Route currConfig : theItems) {
      theList.add(new PlatformObject(currConfig.getMetadata().getUid(),
          currConfig.getMetadata().getName(), "ROUTE"));
    }
    return theList;
  }

  public void deletePlatformObject(String gameID, String objectID, String objectType,
      String objectName) {
